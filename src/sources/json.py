    ...
    def fetch(self) -> Iterable[Message]:
        with self.path.open("r", encoding="utf-8") as file:
            for line_no, line in enumerate(file, start=1):
                ...
                if not line:
                    continue
                data = parse_json_file(line, str(self.path), line_no)
                message_id = str(data.get("id", f"{self.path.name}:{line_no}"))
                title = data.get("title", "")
                author = data.get("author", "")
                content = data.get("message", "")  # исправлено: "message", а не "content"
                yield Message(
                    id=message_id, 
                    title=title, 
                    author=author, 
